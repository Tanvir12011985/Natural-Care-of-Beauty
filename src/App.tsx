...
const getSalePrice = (product: Product) => {
  if (product.isOffer && product.discountPercentage && product.discountPercentage > 0) {
    return Math.round(product.price * (1 - product.discountPercentage / 100));
  }
  return product.price;
};

const generateSalesReportPDF = (orders: Order[]) => {
  const doc = new jsPDF('l', 'mm', 'a4'); // Use landscape for more columns
  const today = new Date().toLocaleDateString();
  
  // Header / Logo area
  // Since we don't have base64 logo easily, we use a styled text header as a fallback
  // If you have a base64 logo, you can use doc.addImage()
  doc.setFillColor(252, 231, 243); // Light pink bg for header
  doc.rect(0, 0, 297, 40, 'F');
  
  doc.setFontSize(28);
  doc.setTextColor(219, 39, 119); // Pink 600
  doc.setFont('helvetica', 'bold');
  doc.text('NATURAL CARE', 148.5, 20, { align: 'center' });
  
  doc.setFontSize(14);
  doc.setTextColor(71, 85, 105); // Slate 600
  doc.setFont('helvetica', 'normal');
  doc.text('Premium Organic Beauty Solutions', 148.5, 28, { align: 'center' });
  
  doc.setFontSize(18);
  doc.setTextColor(15, 23, 42); // Slate 900
  doc.setFont('helvetica', 'bold');
  doc.text('DAILY SALES REPORT', 20, 55);
  
  doc.setFontSize(10);
  doc.setTextColor(100, 116, 139); // Slate 500
  doc.text(`Report Date: ${today}`, 20, 62);
  doc.text(`Generated By: Admin Panel`, 20, 67);

  // Summary Metrics
  const totalRevenue = orders.reduce((acc, o) => acc + o.totalAmount, 0);
  const totalCost = orders.reduce((acc, o) => {
    const orderCost = o.items.reduce((sum, item) => sum + ((item.buyingPrice || 0) * item.quantity), 0);
    return acc + orderCost + (o.deliveryCharge || 0); // Assuming delivery is a cost
  }, 0);
  const totalMargin = totalRevenue - totalCost;

  // Draw simple stats table for summary
  autoTable(doc, {
    startY: 75,
    margin: { left: 20 },
    tableWidth: 100,
    ...
    body: [
      ...
      ['Total Sales Value', `TK ${totalRevenue.toLocaleString()}`],
      ['Total Margin (Profit)', `TK ${totalMargin.toLocaleString()}`]
    ],
  ...
  const tableData = orders.map(order => {
    ...
    const orderCost = order.items.reduce((sum, i) => sum + ((i.buyingPrice || 0) * i.quantity), 0);
    const margin = order.totalAmount - orderCost - (order.deliveryCharge || 0);
  ...
  autoTable(doc, {
    startY: 110,
    head: [['Order #', 'Customer', 'Date/Time', 'Status', 'Items', 'Qty', 'Sales Value', 'Margin']],
    body: tableData,
    foot: [[
      'TOTAL', 
      '', 
      ...
      `TK ${totalMargin.toLocaleString()}`
    ]],
    headStyles: { fillColor: [236, 72, 153], fontSize: 9 }, // Pink 500
    footStyles: { fillColor: [30, 41, 59], textColor: [255, 255, 255], fontSize: 9, fontStyle: 'bold' },
    theme: 'grid',
    styles: { fontSize: 7, cellPadding: 2 },
    columnStyles: {
      4: { cellWidth: 60 } // Items list column needs more width
    }
  });
...
export default function App() {
  const [activeTab, setActiveTab] = useState<'home' | 'store' | 'admin' | 'checkout' | 'success' | 'tracking'>('home');
  ...
  const handleOrderSubmit = async (e: React.FormEvent) => {
    ...
    const newOrderData: Omit<Order, 'id'> = {
      ...
      items: cart.map((item) => {
        ...
        return { ...rest, price: getSalePrice(item) };
      }),
      dateFields: {
  ...
  const cartTotal = cart.reduce((sum, item) => sum + (getSalePrice(item) * item.quantity), 0);
      ...
      <aside className="w-64 bg-slate-900 flex flex-col text-white shrink-0">
        ...
        <nav className="flex-1 py-6 px-4 space-y-2">
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md font-medium transition-colors ${activeTab === 'home' ? 'bg-pink-600/10 text-pink-400' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
          >
            ...
            {activeTab === 'home' && (
              ...
              >
                ...
                {ads.filter(a => a.active).length > 0 && (
                  <div className="relative overflow-hidden rounded-[40px] bg-slate-900 h-[450px] group shadow-2xl">
                    <div 
                    ...
                    >
                      {ads.filter(a => a.active).map((ad) => (
                        <div key={ad.id} className="min-w-full h-full relative">
                          ...
                          />
                          <div className="absolute inset-0 flex flex-col justify-center px-8 md:px-24 text-white bg-gradient-to-r from-slate-900/80 via-slate-900/40 to-transparent">
                            <motion.span 
                              initial={{ opacity: 0, x: -30 }}
                              whileInView={{ opacity: 1, x: 0 }}
                              className="text-[12px] font-black uppercase tracking-[0.6em] text-pink-400 mb-6 drop-shadow-sm"
                            >
                              New Arrival & Offers
                            </motion.span>
                            <motion.h2 
                              ...
                              className="text-4xl md:text-7xl font-black italic tracking-tighter mb-6 max-w-3xl leading-[0.9] drop-shadow-2xl"
                            >
                              {ad.title}
                            </motion.h2>
                            <motion.p 
                              initial={{ opacity: 0, y: 30 }}
                              whileInView={{ opacity: 1, y: 0 }}
                              className="text-slate-200 text-sm md:text-xl font-medium max-w-xl mb-12 leading-relaxed opacity-80"
                            >
                              {ad.description}
                            </motion.p>
                            {ad.linkUrl && (
                              <div className="flex gap-4">
                                <button 
                                  ...
                                  className="px-10 py-5 bg-pink-500 text-white rounded-2xl text-[12px] font-black uppercase tracking-widest hover:bg-white hover:text-slate-900 transition-all shadow-2xl shadow-pink-500/20 active:scale-95 group/btn flex items-center gap-3"
                                >
                                  Explore Collection
                                ...
                                <button 
                                  onClick={() => setActiveTab('store')}
                                  className="px-10 py-5 bg-white/10 backdrop-blur-md text-white border border-white/20 rounded-2xl text-[12px] font-black uppercase tracking-widest hover:bg-white/20 transition-all"
                                >
                                  Browse Store
                                </button>
                              </div>
                    ...
                    {/* Carousel Indicators */}
                    {ads.filter(a => a.active).length > 1 && (
                        ...
                        {ads.filter(a => a.active).map((_, idx) => (
                          <button 
                            ...
                            className={`h-2 rounded-full transition-all duration-500 ${
                              currentAdIndex === idx ? 'w-12 bg-pink-500 shadow-lg shadow-pink-500/50' : 'w-3 bg-white/20 hover:bg-white/40'
                            }`}
                          />
                        ))}
                      </div>
                ...
                <div className="space-y-8">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-6">
                    <div>
                      <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase italic">Best Sellers</h2>
                      <p className="text-slate-500 text-xs mt-1 font-medium tracking-wide">Our most loved natural beauty essentials.</p>
                    </div>
                    <button 
                      onClick={() => setActiveTab('store')}
                      className="group flex items-center gap-2 text-[11px] font-black text-pink-500 uppercase tracking-widest hover:text-slate-900 transition-all"
                    >
                      View All Products
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {[...productsList]
                      ...
                      <div key={product.id} className="group cursor-pointer" onClick={() => setActiveTab('store')}>
                        <div className="aspect-[3/4] bg-white rounded-[32px] overflow-hidden border border-slate-100 shadow-sm transition-all group-hover:shadow-xl group-hover:-translate-y-2 group-hover:border-pink-100 relative">
                          <img 
                            src={product.imageUrl} 
                            alt={product.name}
                            className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                          />
                          {product.isOffer && product.discountPercentage && product.discountPercentage > 0 && (
                            <div className="absolute top-6 left-6 px-4 py-2 bg-pink-500 text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg shadow-pink-500/30">
                              -{product.discountPercentage}% OFF
                            </div>
                          )}
                        </div>
                        <div className="mt-6 text-center">
                          <h3 className="font-bold text-slate-900 group-hover:text-pink-500 transition-colors uppercase text-[12px] tracking-tight">{product.name}</h3>
                          <div className="flex flex-col items-center mt-2">
                             <div className="flex items-center gap-2">
                               <span className="text-pink-500 font-black">{formatPrice(getSalePrice(product))}</span>
                               {product.isOffer && product.discountPercentage && product.discountPercentage > 0 && (
                                 <span className="text-slate-400 text-[10px] line-through font-medium">{formatPrice(product.price)}</span>
                               )}
                             </div>
                          </div>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-12">
                  <div className="p-8 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <h3 className="font-black text-slate-900 uppercase tracking-tight mb-3">100% Organic</h3>
                    <p className="text-slate-500 text-xs leading-relaxed font-medium">We source the purest botanical ingredients from verified sustainable farms.</p>
                  </div>
                  <div className="p-8 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                    <div className="w-12 h-12 bg-pink-50 rounded-2xl flex items-center justify-center mb-6 text-pink-500">
                      <Truck className="w-6 h-6" />
                    </div>
                    <h3 className="font-black text-slate-900 uppercase tracking-tight mb-3">Fast Delivery</h3>
                    <p className="text-slate-500 text-xs leading-relaxed font-medium">Standard delivery inside Dhaka within 48 hours and 72 hours across Bangladesh.</p>
                  </div>
                  <div className="p-8 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                    <div className="w-12 h-12 bg-pink-50 rounded-2xl flex items-center justify-center mb-6 text-pink-500">
                      <ShieldCheck className="w-6 h-6" />
                    </div>
                    <h3 className="font-black text-slate-900 uppercase tracking-tight mb-3">Dermatologist Tested</h3>
                    <p className="text-slate-500 text-xs leading-relaxed font-medium">Every product is tested for safety and efficacy to ensure the best results.</p>
                  </div>
                </div>
              </motion.div>
            )}
                  ...
                  {filteredProducts.map((product) => (
                    <div key={product.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm group hover:shadow-md transition-all hover:border-pink-100">
                      <div className="aspect-[4/5] bg-slate-100 relative overflow-hidden">
                        ...
                        {product.isOffer && product.discountPercentage && product.discountPercentage > 0 && (
                          <div className="absolute top-3 right-3 px-2 py-0.5 bg-pink-500 text-white rounded-[4px] text-[9px] font-black uppercase tracking-widest shadow-lg shadow-pink-500/20">
                      ...
                      <div className="p-5">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex flex-col">
                            <span className="text-pink-600 font-black text-base">{formatPrice(getSalePrice(product))}</span>
                            {product.isOffer && product.discountPercentage && product.discountPercentage > 0 && (
                              <span className="text-[10px] text-slate-400 line-through font-bold">{formatPrice(product.price)}</span>
                            )}
                          </div>
                          <button 
              ...
              >
                ...
                {adminSubTab === 'overview' && (
                   <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                    <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                      <div>
                        <h1 className="text-xl font-black text-slate-900 tracking-tight uppercase tracking-widest">Analytics Overview</h1>
                        <p className="text-slate-500 text-[10px] font-bold uppercase mt-1 tracking-wider">Business growth metrics and daily sales performance</p>
                      </div>
                      <button 
                        onClick={() => {
                          ...
                          generateSalesReportPDF(todaysOrders);
                        }}
                        className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-pink-600 transition-all shadow-xl shadow-slate-900/10"
                      >
                        <Download className="w-4 h-4" /> Today's PDF Report
                      </button>
                    </div>
                        ...
                        <div className="flex items-end">
                          <button 
                             onClick={() => {
                               ...
                               generateSalesReportPDF(dayOrders);
                             }}
                             className="bg-slate-900 border border-slate-900 text-white rounded-xl px-4 py-2.5 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-pink-600 hover:border-pink-600 transition-all flex items-center gap-2 shadow-xl shadow-slate-900/10"
                           >
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                  ...
                  >
                    ...
                    <form onSubmit={handleProductSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            <input 
                              ...
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 transition-all outline-none"
                            />
                            {productForm.isOffer && productForm.discountPercentage > 0 && (
                              <div className="absolute right-4 top-1/2 -translate-y-1/2 bg-pink-100 text-pink-600 px-2 py-1 rounded text-[10px] font-black">
                                Sale: {formatPrice(Math.round(productForm.price * (1 - productForm.discountPercentage / 100)))}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-8 bg-pink-50/50 p-6 rounded-2xl border-2 border-pink-100 transition-all">
                        <div className="flex items-center gap-4">
                          <input 
                            type="checkbox" 
                            id="isOffer"
                            checked={productForm.isOffer}
                            onChange={(e) => setProductForm(prev => ({ ...prev, isOffer: e.target.checked }))}
                            className="w-6 h-6 text-pink-600 border-pink-300 rounded-md focus:ring-pink-500 cursor-pointer"
                          />
                          <label htmlFor="isOffer" className="text-sm font-black text-pink-700 uppercase tracking-widest cursor-pointer select-none">Special Offer</label>
                        </div>
                        {productForm.isOffer && (
                          <div className="space-y-2">
                            <label className="text-[11px] font-black text-pink-400 uppercase tracking-[0.2em] ml-1">Discount (%)</label>
                            <div className="relative">
                              <input 
                                type="number" 
                                placeholder="0"
                                ...
                                className="w-full bg-white border-2 border-pink-200 rounded-xl px-5 py-4 text-lg font-black text-pink-600 focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 outline-none transition-all shadow-sm"
                              />
                              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-pink-300 font-black text-xl">%</span>
                            </div>
                          </div>
                        )}
                      </div>
